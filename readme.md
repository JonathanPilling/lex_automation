...
2. You should see a screen that says, **Welcome to Jenkins!** type the following username and password:
   *Username*: **admin**
   *Password*: **admin**
   and click **Sign in**
3. You should see the main Jenkins page with a list of projects. To run an automation suite, click on a project
4. On the project screen, click **Build Now** on the left
5. A few seconds later you'll see a flashing build icon under **Build History**
...
1. For the time being, figuring out why things failed is a little tricky. Hopefully this will be made a little easier in the future. Click on a pass/failed project, indicated by a blue/red circle
...
Or this, for a fail:
The automation suite makes use of the RetryAnalyzer class, so tests will run 5x before they are flagged as a fail. If a test is failing here, it means it failed 5x in a row and likely something is wrong with the website or there was a change that broke the framework. You can read the test name to get a better idea of what went wrong. 
...
4. Add any recipients you want in the **Recipients** text field
